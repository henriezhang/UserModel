...
/**
 * Created by henriezhang on 2015/1/7.
 */
public class UserUviuvInterest {
    ...
    public static class SplitCombiner
        ...
        public void reduce(Text Id, Iterable<Text> values,
            ...
            while (it.hasNext()) {
                Map.Entry entryColumn = (Map.Entry) it.next();
        ...
        public void reduce(Text Id, Iterable<Text> values,
            ...
            while (it.hasNext()) {
                ...
                if (num != null) {
                    ...
                    // 新版数据
                    double self_rate = (double)((Long)entryColumn.getValue() / all[cls.length]);
                    double site_rate = ((Double)num).doubleValue();
                    double r = 1.0f / site_rate;
                    r = (r>1000) ? 1000 : r;
                    double w = self_rate * (Math.log(r) / 0.6931);  // Math.log(2.0))=0.6931
                    weight += "," + key + ":" + df.format(w);
                }
            }
            if (weight.length() > 1) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job job = new Job(conf, "UserUviuvInterest");
